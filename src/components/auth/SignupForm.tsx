...
function SignupForm() {
    ...
    const handleSubmit = async (e: any) => {
        e.preventDefault();
        setError("");
        if (!data.name || !data.email || !data.password) return setError("All fields are required.");
        setLoading(true);
        ...
        <div className="relative flex w-full flex-col justify-center px-8 py-12 lg:w-[480px] lg:min-w-[420px] lg:px-14">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                    ...
                    <Input type="text" placeholder="Felix Warano" value={data.name} onChange={(e) => setData((prev) => ({ ...prev, name: e.target.value }))} className="border-white/10 bg-white/5 text-white placeholder:text-white/20 focus-visible:border-white/35 focus-visible:ring-0 transition-all" />
                </div>
            ...
            <p className="mt-6 text-center text-sm text-white/35">
                ...
                <a href="/auth/login" className="cursor-pointer font-semibold text-white/75 transition-colors hover:text-white">
                    Sign in
                </a>
            </p>
        </div>
